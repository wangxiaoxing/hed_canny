...
def pred(test_images):
    ...
    for test_image in test_images:
        ...
        # cv2.waitKey(0)
        # cv2.imwrite('./hed_test_result/' + test_image + 'gray.jpg', output_img)
        # cv2.imwrite('./hed_test_result/' + test_image + 'black.jpg', 255 * (output_img > 127))
    # # 滤波处理
    # img = cv2.imread('./tb_black_img.png')
    # img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # img_median = cv2.medianBlur(img, 5)
    # cv2.imshow('median',img_median)
    # [rows, cols] =img_median.shape
    # print(rows, cols)
    # canny_points = []
    # for i in range(rows):
    #     for j in range(cols):
    #         if img_median[i, j] == 255:
    ...
    #     # print(point[0])
    #     # print(point[1])
    #     cv2.circle(img_median, (point[0], point[1]), 6, (255, 0, 0))
    # cv2.imshow('minAreaRect',img_median)
    # cv2.waitKey()
    # # 调用sift+houglines算法
    # points=SIFT_HoughLines(img_median)
    # print(points)
    # cv2.waitKey(0)
    sess.close()
...
#     # 读取测试文件夹下的所有文件
#     test_images = ['./hed_test_data_resize/14.jpg']
#     pred(test_images)
